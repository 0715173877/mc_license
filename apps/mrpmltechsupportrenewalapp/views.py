...
from .serializers import (
    DraftPmlTechSupportRenewalSerializer,
...
# ---------- Draft CRUD ----------
class DraftPmlTechSupportRenewalViewSet(viewsets.ModelViewSet):
    queryset = DraftPmlTechSupportRenewal.objects.all()
    serializer_class = DraftPmlTechSupportRenewalSerializer
...
# ---------- Final PML Technical Support Renewal ----------
class PmlTechnicalSupportRenewalViewSet(viewsets.ModelViewSet):
    queryset = PmlTechnicalSupport.objects.all()
    serializer_class = PmlTechnicalSupportSerializer

    def get_queryset(self):
        # Return only renewals
        return PmlTechnicalSupport.objects.filter(is_fresh_application=False)
    ...
    # ---------- 1. Direct creation ----------
    def create(self, request, *args, **kwargs):
        # Force is_fresh_application = False
        if 'is_fresh_application' not in request.data:
            request.data['is_fresh_application'] = False
    ...
    def submit(self, request, pk=None):
        ...
        if pml_ts.status_id == 2:
            return Response(
                {'error': 'PML Technical Support renewal already submitted.'},
                status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def submit_from_draft(self, request):
        ...
        try:
            draft = DraftPmlTechSupportRenewal.objects.get(pk=draft_id)
        except DraftPmlTechSupportRenewal.DoesNotExist:
            return Response(
        ...
        data = draft.data
        data['is_fresh_application'] = False
